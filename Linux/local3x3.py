...
def gameplay(K,num):
    ...
    return cont
class btn:
        def __init__(self,ascissa,ordinata,num,master):#its just a way to write btn propreties
                self.num = num
                self.master = master
                self.ascissa = ascissa
                self.ordinata = ordinata
                self.Button = None
        def callback(self,num):#already tryead outside
            ...
            if check(pl1,num) == True or check(pl2,num) == True:
                self.Button.config(command=0)
            #normal game (i separated it for other propouses)
            else:
                    gameplay(self.Button,num)
...
def start(l,a1,a2,buttons): #main play and reset values for win possibilities
...
#adding numbers will increase games possibilities,(new tabs 4x4,5x5)(need to change also the table)
        win = [(1,2,3),(4,5,6),(7,8,9),(1,4,7),(2,5,8),(3,6,9),(1,5,9),(3,5,7)]
        turn = turn + 1
        buttons.append(btn(0,0,1,master).create())
        buttons.append(btn(0,1,2,master).create())
        buttons.append(btn(0,2,3,master).create())
        buttons.append(btn(1,0,4,master).create())
        buttons.append(btn(1,1,5,master).create())
        buttons.append(btn(1,2,6,master).create())
        buttons.append(btn(2,0,7,master).create())
        buttons.append(btn(2,1,8,master).create())
        buttons.append(btn(2,2,9,master).create())
